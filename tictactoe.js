...
Elm.TTTModel.make = function (_elm) {
   ...
   var nextGame = function (model) {
      return function () {
         var player = other(turn(function (_) {
            return _.state;
         }(model)));
...
Elm.TicTacToe.make = function (_elm) {
   ...
   _L.fromArray([$Html.text("2015 - Alex Kelley")]))]));
   var blinkTurn = F2(function (player,
   turn) {
      return _U.eq(player,
      turn) ? "blink" : "no-blink";
   });
   var panelLabel = F3(function (id$,
   class$,
   label) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.id(id$)]),
   ...
   points) {
      ...
      _L.fromArray([]),
      _L.fromArray([renderScore(points.o)
   ...
   var title = function (message) {
      return $Html.text($String.toUpper(message));
   };
   var pageHeader = A2($Html.h1,
   _L.fromArray([$Html$Attributes.id("header")]),
   _L.fromArray([title("tic-tac-toe")]));
      ...
      return function () {
         ...
         {case "AddMove":
            return $Debug.log("Add move")(A2($TTTModel.addMove,
              ...
              action._0));
            case "Next":
            return $TTTModel.nextGame(model);
            case "NoOp": return model;
            case "Reset":
            return $TTTModel.resetModel;}
         _U.badCase($moduleName,
   ...
   model) {
      ...
      _L.fromArray([pageHeader
                   ...
                   ,A2($Html.button,
                   _L.fromArray([$Html$Attributes.id("next-game")
                                ,A2($Html$Events.onClick,
                                address,
                                Next)]),
                   _L.fromArray([$Html.text("Next Game")]))
                   ,pageFooter]));
   });
   var main = $StartApp.start({_: {}
                              ,model: $TTTModel.resetModel
                              ,update: update
                              ,view: view});
   var NoOp = {ctor: "NoOp"};
